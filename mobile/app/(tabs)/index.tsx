...
export const useRecord = create((set) => ({
  ...
  fetchRecords: (date: Date, session: any) => {
    ...
    axios.post(`${process.env.EXPO_PUBLIC_API_URL}/records`, {
      ...
      date: date.toISOString().split('T')[0] // 只需要日期部分 YYYY-MM-DD
    }, { headers }).then((res) => {
...
export default function HomeScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 flex gap-4 mx-4">
      ...
      {/* 详细记录 */}
      <View className="flex-1 bg-grey-100 rounded-lg py-4 gap-2">
        <Text className="text-gray-500">详细记录</Text>
        <ScrollView className="flex-1">
          {/* <RecordCard record={{ id: 1, title: "工资", amount: 5000, createdAt: "2023-10-01" }} /> */}
          {records.map((record: any) => (
            <RecordCard key={record.id} record={record} />
          ))}
          {/* 调试信息 */}
          {/* <Text>Records 类型: {typeof records}</Text>
          <Text>Records 值: {JSON.stringify(records)}</Text> */}
        </ScrollView>
      </View>
    </SafeAreaView>
  )
